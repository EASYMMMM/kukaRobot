...
if GPR_ENABLE
    ...
    all_traj_GPR = [];
    all_target_point=[];all_this_intent=[];all_lock_state=[];
end
...
%%  please prepare for starting  定义一些存储变量
...
while OVER == 0
    if GPR_ENABLE
        ...
        %% GPR send data
        if high_loop > 1 && round_over == 1  %% because last trajectory should be whole finished
            [row,col]=size(temp_hand);
            %         all_data = [all_delta_pos_hand; all_delta_v_hand]';
            listed_1=reshape(temp_hand,row*col,1);
            %         sb=reshape(listed_1,6,length(listed_1)/3);
            index_obs=mode(temp_state_label(1,end-10:end));
...
%         LABEL_ENABLE=1;
    ...
    if GPR_ENABLE
        % GPR 调整规划的终点的高度，这里只列举了v-state=2的情况
        if start_end_points(3,2) > 0.4 && target_point_obs1_pose2 ~= 0 && temp_state_label(2,end) == 2
            start_end_points(3,2) = target_point_obs1_pose2;
            disp('GPR起作用之后')
            start_end_points
        end

        if start_end_points(3,2) < 0.4 && target_point_obs2_pose2 ~= 0 && temp_state_label(2,end) == 2
            start_end_points(3,2) = target_point_obs2_pose2;
            disp('GPR起作用之后')
            start_end_points
        end
    end
...
%     end
    ...
    F_filtered3_old = kalmanz(F_contact(3)) ;
    F_filtered3 = F_filtered3_old + 5;
    % 
    if F_filtered3 < -3
        F_filtered3 = F_filtered3 + 3;
    elseif F_filtered3 > 3
        F_filtered3 = F_filtered3 - 3;
    else
        F_filtered3 = 0;
    end
...
%% real time interaction
    ...
    if GPR_ENABLE
        if  t_server_GPR.BytesAvailable>0
            ...
            if this_obs == 1
                if pose_recv == 1
                    target_point_obs1_pose1=after_T(end)
                elseif pose_recv == 2
                    target_point_obs1_pose2=after_T(end)
                elseif pose_recv == 3
                    target_point_obs1_pose3=after_T(end)
                end
         
            elseif this_obs == 2
                if pose_recv == 1
                    target_point_obs2_pose1=after_T(end)
                elseif pose_recv == 2
                    target_point_obs2_pose2=after_T(end)
                elseif pose_recv == 3
                    target_point_obs2_pose3=after_T(end)
                end                

%% Example of using KST class for interfacing with KUKA iiwa robots 简单测试  。。。
...
pause(1);
time = clock;
disp(['Go to initial configuration. Time: ',num2str(time(4)),':',num2str(time(5)),':',num2str(time(6))]);

iiwa.movePTPJointSpace( pos, relVel); % go to initial position

time = clock;
disp(['Acheive initial configuration. Time: ',num2str(time(4)),':',num2str(time(5)),':',num2str(time(6))]);

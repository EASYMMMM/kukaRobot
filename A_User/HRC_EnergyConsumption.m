...
timeVec  = [0:timeInt:runTime];    
totalLoop = length(timeVec);
timepoint = linspace(0,runTime,3);
% 低层
LowWayPoint(:,1)=[ 0.70 ; 0.00 - 0.20 ; 0.3 ];
LowWayPoint(:,2)=[ 0.70 ; 0.00 + 0.20 ; 0.3 ];
LowWayPoint(:,3)=[ 0.70 ; 0.00 - 0.20 ; 0.3 ];
%中层
MidWayPoint(:,1)=[ 0.70 ; 0.00 - 0.20 ; 0.4 ];
MidWayPoint(:,2)=[ 0.70 ; 0.00 + 0.20 ; 0.4 ];
MidWayPoint(:,3)=[ 0.70 ; 0.00 - 0.20 ; 0.4 ];
%上层、
HighWayPoint(:,1)=[ 0.70 ; 0.00 - 0.20 ; 0.5 ];
HighWayPoint(:,2)=[ 0.70 ; 0.00 + 0.20 ; 0.5 ];
HighWayPoint(:,3)=[ 0.70 ; 0.00- 0.20 ; 0.5 ];
%% 存储数据
stopdq = [ 0 ; 0 ; 0;0;0;0;0];  %刹车
jPosdLast = stopdq;
...
%高导纳参数
k_cartesian_high = diag([100,100,150]*1*1)*1.3*5*2/2;
b_cartesian_high = diag([100,100,150]*2.5);
...
for RobotState = 1:3
    ...
    switch RobotState   %低层 或 中层 或 高层
        case 1 
            waypoint = LowWayPoint;
            LOOP = [1 2];
        case 2
            waypoint = MidWayPoint;
            LOOP = [1 2 3];
        case 3
            waypoint = HighWayPoint;
            LOOP = [2 3];
    end
    [eefTarget ,eefTargetd ,eefTargetdd, pp] = cubicpolytraj(waypoint,timepoint ,timeVec);
    xd=waypoint(1,1);
    ...
    iiwa.realTime_startVelControlJoints(); %开启实时控制
    ...
    for Loop = LOOP 
        ...
        switch Loop   %存储能耗数据
            case 1 %HumanHigher 
                switch RobotState
                    case 1
                        RobotLow_EnergyCostHumanHigher  = totalMusclePower / runTime ; 
                    case 2
                        RobotMid_EnergyCostHumanHigher  = totalMusclePower / runTime ; 
                    case 3
                        RobotHigh_EnergyCostHumanHigher  = totalMusclePower / runTime ; 
                end
            case 2
                switch RobotState
                    case 1
                        RobotLow_EnergyCostHorizontal  = totalMusclePower / runTime ; 
                    case 2
                        RobotMid_EnergyCostHorizontal  = totalMusclePower / runTime ; 
                    case 3
                        RobotHigh_EnergyCostHorizontal  = totalMusclePower / runTime ; 
                end                
            case 3
                switch RobotState
                    case 1
                        RobotLow_EnergyCostRobotHigher  = totalMusclePower / runTime ; 
                    case 2
                        RobotMid_EnergyCostRobotHigher  = totalMusclePower / runTime ; 
                    case 3
                        RobotHigh_EnergyCostRobotHigher  = totalMusclePower / runTime ; 
                end  
        end
...
%% Save data
save(fileName, 'RobotLow_EnergyCostHumanHigher','RobotLow_EnergyCostHorizontal', ...
    'RobotMid_EnergyCostHumanHigher','RobotMid_EnergyCostHorizontal','RobotMid_EnergyCostRobotHigher',...
    'RobotHigh_EnergyCostHorizontal','RobotHigh_EnergyCostRobotHigher','ALL_EMG_data');
disp('数据已存储！');

%% Plot 
RobotLow_EnergyCostHorizontal
return
xx = [0,1];
figure(2)
plot( xx, [RobotLow_EnergyCostHumanHigher RobotLow_EnergyCostHumanHigher],'b','Linewidth',2);
hold on
plot( xx, [RobotLow_EnergyCostHorizontal RobotLow_EnergyCostHorizontal],'g','Linewidth',2);
grid on 
axis([0 1 0 1])
title('Robot Low State ');
legend('HumanHigher','Horizontal');
hold off


figure(3)

plot( xx, [RobotMid_EnergyCostHumanHigher RobotMid_EnergyCostHumanHigher],'b','Linewidth',2);
hold on
plot( xx, [RobotMid_EnergyCostHorizontal RobotMid_EnergyCostHorizontal],'g','Linewidth',2);
plot( xx, [RobotMid_EnergyCostRobotHigher RobotMid_EnergyCostRobotHigher],'r','Linewidth',2);
grid on
title('Robot Mid State ');
legend('HumanHigher','Horizontal','RobotHigher');
axis([0 1 0 1])

figure(4)
plot( xx, [RobotHigh_EnergyCostHorizontal RobotHigh_EnergyCostHorizontal],'g','Linewidth',2);
hold on
plot( xx, [RobotHigh_EnergyCostRobotHigher RobotHigh_EnergyCostRobotHigher],'r','Linewidth',2);
grid on 
axis([0 1 0 1])
title('Robot Low State ');
legend('HumanHigher','Horizontal');

function [data cartis_obs]= get_init_big_maze_v1(input)
% 开始折磨队友，目前的pose有
...
for j = 1:11
    data{4*11+j,3}(1)=data{4*11+j,3}(1)+0.07;
    data{10*11+j,3}(1)=data{10*11+j,3}(1)+0.07;
end
for j = 4:8
    data{3*11+j,3}(1)=data{3*11+j,3}(1)+0.07;
    data{9*11+j,3}(1)=data{9*11+j,3}(1)+0.07;
end

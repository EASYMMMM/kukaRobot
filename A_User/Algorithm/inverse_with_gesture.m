...
function [All_theta] = inverse_with_gesture(xd,yd,zd,input_theta1,input_theta2)
%A1 ±170 °
%A2 ±120 °
%A3 ±170 °
%A4 ±120 °
...
Rx=[ 
     1        0            0;
     0   cosd(input_theta1)   -sind(input_theta1);
     0   sind(input_theta1)    cosd(input_theta1)];
 
Ry=[  cosd(input_theta2)    0    sind(input_theta2);
      0           1    0;
     -sind(input_theta2)    0    cosd(input_theta2)];
 Rd=Rx*Ry;
Ocd=Od-(dn*Rd*[0 0 1]');
...
%     output_order=['theta_1','  theta_2','  theta_3','  theta_4','  theta_5','  theta_6','  theta_7']
    theta=[0,0,0,0,0,0,0];
    new_theta=[0,0,0,0,0,0,0];
    sol=0;
    n=1;
    if ~isreal(t2a) || ~isreal(t2b)
...
%                             disp('1_Equal 1-7 sol1')
                            ...
                            if theta(1) < 0
                                new_theta(1)=theta(1)+180;
                            else
                                new_theta(1)=theta(1)-180;
                            end
                            new_theta(2)=theta(2)*(-1);
                            new_theta(3)=theta(3);
                            new_theta(4)=theta(4)*(-1);
                            new_theta(5)=theta(5)*(1);
                            new_theta(6)=theta(6)*(-1);
                            if theta(7) < 0
                                new_theta(7)=theta(7)+180;
                            else
                                new_theta(7)=theta(7)-180;
                            end
